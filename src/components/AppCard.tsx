"use client";

import * as React from "react";
import Image from "next/image";
import Link from "next/link";
import { Badge } from "./Badge";
import { ArrowUpRight, CheckCircle } from "lucide-react";
import type { AppData } from "@/lib/constants";

interface AppCardProps {
  app: AppData;
}

// User-friendly platform names
const platformNames: Record<number, string> = {
  33529: "Varity",
  421614: "Arbitrum Test",
  42161: "Arbitrum",
  8453: "Base",
  137: "Polygon",
  10: "Optimism",
  1: "Ethereum",
};

function AppCardComponent({ app }: AppCardProps): React.ReactElement {
  const platformId = Number(app.chainId);
  const platformName = platformNames[platformId] || "Other";

  return (
    <Link
      href={`/app/${app.id.toString()}`}
      className="group card card-hover relative flex flex-col"
      aria-label={`View details for ${app.name} - ${app.category} application`}
    >
      {/* Logo & Name */}
      <div className="flex items-start gap-4">
        <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded-xl bg-background-tertiary">
          {app.logoUrl ? (
            <Image
              src={app.logoUrl}
              alt={`${app.name} logo`}
              width={56}
              height={56}
              className="object-cover"
              loading="lazy"
              sizes="56px"
            />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-xl font-bold text-brand-400" aria-hidden="true">
              {app.name.charAt(0).toUpperCase()}
            </div>
          )}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-heading-md text-foreground transition-colors group-hover:text-brand-400">
              {app.name}
            </h3>
            {app.builtWithVarity && (
              <CheckCircle className="h-4 w-4 flex-shrink-0 text-brand-500" />
            )}
          </div>
          <p className="mt-1.5 line-clamp-2 text-body-sm text-foreground-secondary">
            {app.description}
          </p>
        </div>
      </div>

      {/* Metadata */}
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <Badge variant="default">{app.category}</Badge>
        <Badge variant="secondary">{platformName}</Badge>
        {app.builtWithVarity && (
          <Badge variant="success">Verified</Badge>
        )}
      </div>

      {/* Hover indicator */}
      <div className="absolute right-5 top-5 opacity-0 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">
        <ArrowUpRight className="h-5 w-5 text-brand-400" aria-hidden="true" />
      </div>
    </Link>
  );
}

// Memoize to prevent unnecessary re-renders when parent updates
export const AppCard = React.memo(AppCardComponent);
